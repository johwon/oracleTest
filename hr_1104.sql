...
SELECT FIRST_NAME, LAST_NAME, SALARY, COMMISSION_PCT, EMAIL
FROM EMPLOYEES
WHERE EMAIL LIKE UPPER('jking');
...
SELECT *
FROM EMPLOYEES
WHERE SUBSTR(TO_CHAR(HIRE_DATE),1,2)=3;

SELECT *
FROM EMPLOYEES
WHERE TO_NUMBER(SUBSTR(TO_CHAR(HIRE_DATE),1,2))=3;
...
SELECT FLOOR(SYSDATE - TO_DATE('2024/11/05')) FROM DUAL;

--숫자를 우리가 원하는 형식으로 출력하기 1234567.89 => $1,234,567.89
...
--NVL
SELECT FIRST_NAME, SALARY, NVL(COMMISSION_PCT,0) AS COMMSSION_PCT, JOB_ID
FROM EMPLOYEES
ORDER BY JOB_ID;

--NVL2(대상, NULL아닐때 적용될 값, NULL일때 적용될 값)
