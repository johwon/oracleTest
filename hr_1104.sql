...
SELECT DEPARTMENT_ID, MAX(SALARY), MIN(SALARY), ROUND(AVG(SALARY),1), COUNT(SALARY), SUM(SALARY) 
FROM EMPLOYEES 
...
--20번 부서에서 사원들의 HIRE_DATA 년도 가져오기
SELECT EMPLOYEE_ID, FIRST_NAME, SUBSTR(HIRE_DATE, 1, 2)||'년도' AS 입사년도 
FROM EMPLOYEES WHERE DEPARTMENT_ID = 20;

--근무달수를 구하기
SELECT first_name, last_name, HIRE_DATE AS 입사일, SYSDATE AS 현재날짜, ROUND(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) AS 근무달수 
FROM EMPLOYEES
