...
--PL/SQL
--EMPLOYEES 테이블에서 요구한 부서별로 사원 정보(이름,월급)를 CURSOR에 저장하고 부서별 요청시
--해당되는 부서정보를 출력
...
BEGIN
    ...
    IF (VNO=40) THEN
        ...
        RETURN;
    END IF;
    --부서번호 정보를 가져와서 월급에 대해서 평가를 진행한다(1~1000 낮음 1000~2000중간 2000~3000높음 상위)
    FOR VEMP_ROWTYPE IN C1(VNO) LOOP
        ...
        ELSIF VEMP_ROWTYPE.SALARY BETWEEN 1001 AND 2000 THEN
            VSALARY := '중간';
        ELSIF VEMP_ROWTYPE.SALARY BETWEEN 2001 AND 1000 THEN
            VSALARY := '높음';
        ELSE
            VSALARY := '상위';  
        END IF;
